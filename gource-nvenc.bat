...
if "%~1" == "" (
    echo Usage: %~nx0 output-file.mkv
    echo        %~nx0 logo-file output-file.mkv
    exit /b 1
)
...
if "%~2" == "" (
    for /f "usebackq" %%g in (`git config --get remote.origin.url`) do set "git_url=%%g"
    REM -2560x1440 # If not --fullscreen # --date-format "%Y-%m-%d" --elasticity 0.1 --max-user-speed 500
    REM FFMPEG -pix_fmt yuv420p yuvj420p rgb0
